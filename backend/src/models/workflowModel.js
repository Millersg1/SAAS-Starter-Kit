...
export const getWorkflows = async (brandId) =>
  (await query(
    `SELECT w.*, COUNT(e.id) FILTER (WHERE e.status='active') as active_enrollments
...
export const createWorkflow = async ({ brand_id, name, trigger_type, trigger_config, created_by }) =>
  (await query(
    `INSERT INTO automation_workflows (brand_id, name, trigger_type, trigger_config, created_by) VALUES ($1,$2,$3,$4,$5) RETURNING *`,
    [brand_id, name, trigger_type, trigger_config || {}, created_by]
  )).rows[0];

export const updateWorkflow = async (id, data) => {
  const allowed = ['name', 'trigger_type', 'trigger_config', 'is_active'];
...
export const enrollEntity = async ({ workflow_id, brand_id, entity_id, entity_type, next_step_at }) =>
  (await query(
    `INSERT INTO automation_enrollments (workflow_id, brand_id, entity_id, entity_type, next_step_at)
     VALUES ($1,$2,$3,$4,$5) RETURNING *`,
    [workflow_id, brand_id, entity_id, entity_type || 'client', next_step_at || new Date()]
  )).rows[0];
